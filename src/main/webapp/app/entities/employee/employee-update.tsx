...
import { getEntities as getKanbams } from 'app/entities/kanbam/kanbam.reducer';
...
export class EmployeeUpdate extends React.Component<IEmployeeUpdateProps, IEmployeeUpdateState> {
  ...
  render() {
    const { employeeEntity, employees, kanbams, loading, updating } = this.props;
    ...
    return (
      <div>
        <Row className="justify-content-center">
          <Col md="8">
            <h2 id="miconsuladogatewayApp.employee.home.createOrEditLabel">Create or edit a Employee</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md="8">
            {loading ? (
              ...
              <AvForm model={isNew ? {} : employeeEntity} onSubmit={this.saveEntity}>
                ...
                <AvGroup>
                  <Label for="employee-kanbam">Kanbam</Label>
                  <AvInput id="employee-kanbam" type="select" className="form-control" name="kanbam.id">
                    <option value="" key="0" />
                    {kanbams
                      ? kanbams.map(otherEntity => (
                          <option value={otherEntity.id} key={otherEntity.id}>
                            {otherEntity.id}
                          </option>
                        ))
...
const mapStateToProps = (storeState: IRootState) => ({
  ...
  kanbams: storeState.kanbam.entities,
