...
export class TaskUpdate extends React.Component<ITaskUpdateProps, ITaskUpdateState> {
  ...
  render() {
    const { taskEntity, jobs, kanbams, loading, updating } = this.props;
    ...
    return (
      <div>
        <Row className="justify-content-center">
          <Col md="8">
            <h2 id="miconsuladogatewayApp.task.home.createOrEditLabel">Create or edit a Task</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md="8">
            {loading ? (
              ...
              <AvForm model={isNew ? {} : taskEntity} onSubmit={this.saveEntity}>
                ...
                <AvGroup>
                  <Label for="task-kanbam">Kanbam</Label>
                  <AvInput id="task-kanbam" type="select" className="form-control" name="kanbam.id">
                    <option value="" key="0" />
                    {kanbams
